...
static ChosenBitrate get_chosen_bitrate(MpegVersion version, int samplerate, ChannelMode channel_mode, int kbps, bool padding) {
    static const int mpeg1_bitrates[] = {0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320};
    static const int mpeg2_bitrates[] = {0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160};
    ...
    int index = 14; // default to max
    for (int idx = 1; idx <= 14; ++idx) {
        if (bitrates[idx] == kbps) {
            index = idx;
            break;
        }
    }
    ...
    int unpadded_frame_size = 0;
    if (version == MpegVersion::MPEG1) {
        unpadded_frame_size = (144 * kbps * 1000 / samplerate);
    } else {
        unpadded_frame_size = (72 * kbps * 1000 / samplerate);
    }
    
    int data_size = unpadded_frame_size + (padding ? 1 : 0) - 4 - side_info_size;
    return {kbps, padding, data_size, index};
}
...
void Packer::process(const std::string& input_file, const std::string& output_file) const {
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        size_t start_byte = (frame_main_start >= frame.side_info.main_data_begin) ? (frame_main_start - frame.side_info.main_data_begin) : 0;
        ...
        try {
            if (!this->recompress_huffman) {
                ...
                for (int ch = 0; ch < n_ch; ++ch) {
                    ...
                    auto coeffs = optimizer.decode_quantized_coefficients(frame, orig_cfg, data_reader, frame.header.samplerate);
                    auto best_cfg = optimizer.find_best_config(frame, coeffs, orig_cfg, frame.header.samplerate);
                    ...
                    g.part2_3_length = writer.tell_bit() - out_start;
                    if (i == 20) {
                        printf("Frame 20 Gr%d Ch%d: Orig p23=%d, New p23=%d\n", 
                            gr, ch, side_copy.gr[gr][ch].part2_3_length, g.part2_3_length);
                    }
        ...
        if (total_orig_bytes > 0 && non_silent_count < 20) {
            std::cout << "Frame " << i << " (non-silent #" << non_silent_count << ") -> Orig: " << total_orig_bytes << " bytes, Optimized: " << new_main.size() << " bytes, Failed: " << optimization_failed << std::endl;
            non_silent_count++;
        }
    ...
    int max_bitrate_kbps = (version == MpegVersion::MPEG1) ? 320 : 160;
    int max_frame_size = (version == MpegVersion::MPEG1) ?
                         (144 * max_bitrate_kbps * 1000 / samplerate) + 1 :
                         (72 * max_bitrate_kbps * 1000 / samplerate) + 1;
    int side_info_size = (version == MpegVersion::MPEG1) ?
                         (channel_mode == ChannelMode::Mono ? 17 : 32) :
                         (channel_mode == ChannelMode::Mono ? 9 : 17);
    int max_data_per_frame = max_frame_size - 4 - side_info_size;

    for (int i = static_cast<int>(N) - 1; i >= 0; --i) {
        pads[i] = new_pad;
        int bytes_to_end = optimized_main_data[i].size() + new_pad;
        new_pad = std::max(0, bytes_to_end - max_data_per_frame);
    }
    ...
    if (!start_junk.empty()) {
        out.write((char*)start_junk.data(), start_junk.size());
    }
    ...
    for (int i = (int)N - 1; i >= 0; --i) {
        int space_needed = optimized_main_data[i].size() + current_req;
    ...
    for (size_t i = 0; i < N; ++i) {
        auto& frame = all_frames[i];
        auto& f1_data = optimized_main_data[i];
        ...
        side.main_data_begin = current_reservoir;
        global_main_data.insert(global_main_data.end(), f1_data.begin(), f1_data.end());
        ...
        int bytes_to_store = std::max(0, (int)f1_data.size() + req_for_next - current_reservoir);
        ChosenBitrate bitrate_use = bytes_to_bitrate(version, samplerate, channel_mode, bytes_to_store);
        
        // Removed clamping to original bitrate. VBR allows frames to naturally scale up to 320 kbps if the reservoir demands it.
        
        int data_size = bitrate_use.data_size;
        current_reservoir = current_reservoir + data_size - f1_data.size();
        
        chosen_bitrates[i] = bitrate_use;
        
        if (current_reservoir < 0) {
            std::cout << "WARNING: Frame " << i << " bit reservoir underflow! (" << current_reservoir << ")" << std::endl;
        }
    }
    
    if (current_reservoir > 0) {
        std::vector<uint8_t> end_pad(current_reservoir, 0x00);
    ...
    for (size_t i = 0; i < N_out; ++i) {
        ...
        // Skip optimizing/serializing side info for the Xing header if it's the first frame
        if (has_xing && i == 0) {
            // Retain original behavior without optimizing side info
        }
        ...
        h |= (1 << 17) | (1 << 16);
        h |= (bitrate_use.index << 12) | (get_samplerate_index(version, samplerate) << 10);
        h |= (bitrate_use.padding ? 1 : 0) << 9 | (static_cast<uint32_t>(channel_mode) << 6);
        ...
        h |= (mode_ext << 4);
        h |= (frame.header.copyright ? 1 : 0) << 3;
        h |= (frame.header.original ? 1 : 0) << 2;
        h |= (static_cast<uint32_t>(frame.header.emphasis)) << 0;
        
        head[0] = h >> 24; head[1] = h >> 16; head[2] = h >> 8; head[3] = h;
        ...
        out.write((char*)new_side.data(), new_side.size());
        
        int data_size = bitrate_use.data_size;
        int avail = global_main_data.size() - global_main_data_ptr;
        int write_len = std::min(data_size, avail);
        
        if (write_len > 0) {
            out.write((char*)&global_main_data[global_main_data_ptr], write_len);
            global_main_data_ptr += write_len;
        }
        
        if (data_size > write_len) {
            std::vector<uint8_t> pad(data_size - write_len, 0x00);
            out.write((char*)pad.data(), pad.size());
        }
    }
    
    // Save position before end_junk to calculate total MP3 stream size
    size_t mp3_end_pos = out.tellp();
    
    // Write end junk (ID3v1)
    const auto& end_junk = reader.get_end_junk();
    if (!end_junk.empty()) {
        out.write((char*)end_junk.data(), end_junk.size());
    }
    ...
    if (has_xing && xing_header_file_pos != 0) {
        ...
        if (xing_main_data.size() >= 12) {
            uint32_t flags = (xing_main_data[4] << 24) | (xing_main_data[5] << 16) | 
                             (xing_main_data[6] << 8) | xing_main_data[7];
            ...
            if (flags & 2) { // Bytes field is present
                ...
                size_t physical_bytes_pos = xing_header_file_pos + 4 + side_info_size + bytes_offset;
                
                // Seek back and write the new size
                out.seekp(physical_bytes_pos, std::ios::beg);
                uint8_t bytes_buf[4];
